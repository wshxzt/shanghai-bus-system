...
function App(){
 ...
 useEffect(()=>{if(!roundTrip)return;setStops(activeRoutePlanner.createTour({origin:from,attractions:spots.filter(s=>!s.isHome&&!s.isSchool&&s.id!==from).map(s=>s.id),lines,departureMinute:Math.floor((simSeconds%86400)/60)}));resetRide()},[plannerId]);
 ...
 const changeStart=id=>{setFrom(id);setStops(current=>roundTrip?activeRoutePlanner.createTour({origin:id,attractions:spots.filter(s=>!s.isHome&&!s.isSchool&&s.id!==id).map(s=>s.id),lines,departureMinute:Math.floor((simSeconds%86400)/60)}):current.filter(stop=>stop!==id));resetRide()};
 const planTrip=(start,end)=>{setFrom(start);setStops([end]);setRoundTrip(false);setTrip(null);setProgress(0);setTab('planner')};
 const toggleFullTour=()=>{if(roundTrip){setStops([]);setRoundTrip(false)}else{setStops(activeRoutePlanner.createTour({origin:from,attractions:spots.filter(s=>!s.isHome&&!s.isSchool&&s.id!==from).map(s=>s.id),lines,departureMinute:Math.floor((simSeconds%86400)/60)}));setRoundTrip(true)}resetRide()};
