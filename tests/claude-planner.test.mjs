...
import planner from '../route-planners/claude.js';
...
// createTour-specific: the 2-opt cost matrix must account for the ride home,
// and the final tour must actually be timed against the real schedule.
test('createTour accounts for the return leg and times both directions',()=>{
 const lines=[{id:'loop',stops:['home','a','b','c','home']}];
 const forwardOnly={origin:'home',attractions:['a','b','c'],lines};
 const tour=planner.createTour(forwardOnly);
 assert.deepEqual([...tour].sort(),['a','b','c']);

 // A network where riding the loop backwards catches a much better bus.
 const timedLines=[
  {id:'slow',stops:['home','a','b','c']},
  {id:'express',stops:['c','b','a','home']},
 ];
 const withTime={origin:'home',attractions:['a','b','c'],lines:timedLines,departureMinute:320};
 const timedTour=planner.createTour(withTime);
 const destinations=[...timedTour,'home'];
 const trip=planner.planTrip({origin:'home',destinations,lines:timedLines,departureMinute:320});
 const timedCost=simulateTripTiming(trip,destinations,'home',320,timedLines).timeMinutes;
 const otherDirection=[...timedTour].reverse();
 const otherDestinations=[...otherDirection,'home'];
 const otherTrip=planner.planTrip({origin:'home',destinations:otherDestinations,lines:timedLines,departureMinute:320});
 const otherCost=simulateTripTiming(otherTrip,otherDestinations,'home',320,timedLines).timeMinutes;
 assert.ok(timedCost<=otherCost);
});
